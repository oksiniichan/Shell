...
import { WAIFUS } from './commands/waifus';
...
const COMMANDS = ["help", "about", "contacts", "games", "waifus", "whoami", "banner", "clear"];
...
function commandHandler(input: string) {
  ...
  switch (input) {
    ...
    case 'banner':
      if (bareMode) {
        writeLines(["WebShell v1.26.65", "<br>"])
        break;
      }
      writeLines(BANNER);
      break;
    ...
    case 'about':
      if (bareMode) {
        writeLines(["Want to know about me after breaking shell? XD", "<br>"])
        break;
      }
      writeLines(ABOUT);
      break;
    ...
    case 'extras':
      if (bareMode) {
        writeLines(["You removed the source files XD", "<br>"])
      }
      writeLines(EXTRAS);
      break;
    ...
    case 'games':
      if (bareMode) {
        writeLines(["I don't want you to break this as well.", "<br>"])
        break;
      }
      writeLines(GAMES);
      break;
